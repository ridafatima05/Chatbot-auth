...
from datetime import datetime  # For fetching current date
...
# Decorator to handle OAuth callback from GitHub or Google
@cl.oauth_callback
def oauth_callback(
    provider_id: str,  # ID of the OAuth provider (GitHub, Google)
    ...
    """
    Handle the OAuth callback from GitHub and Google.
...
async def handle_chat_start():
    ...
    await cl.Message(
        content="👋 Hi there! Welcome to Rida’s chatbot — I'm here to help with anything you need. How can I assist you today?"
...
async def handle_message(message: cl.Message):
    ...
    # Check if user is asking about the date
    if "date" in user_text or "today" in user_text:
        today_date = datetime.now().strftime("%d %b %Y")  # Format: 24 May 2025
        await cl.Message(content=f"Today's date is {today_date}.").send()
        return

    # Otherwise proceed with normal Gemini conversation flow
